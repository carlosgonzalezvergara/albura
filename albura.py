...
def draw_lsc_tree(data):
    ...
    # Elements between AUX and PRED (only for copular)
...
with main_c1:
    ...
    with st.expander("1. Nucleus", expanded=False):
        pred_type = st.radio("Type", ["Predicative", "Attributive"], horizontal=True, key="pred_type")
    ...
    with st.expander("2. Constituents", expanded=False):
        ...
        num_pre = st.number_input("Number of items", min_value=0, value=1, key="num_pre")
        items_pre_data = []
        for i in range(num_pre):
            st.markdown(f"**Item {i+1}**")
            conn_type_raw = st.selectbox("Type", ["Argument", "Periphery (NUC)", "Periphery (CORE)", "Periphery (CLAUSE)"], key=f"pre_c_{i}")
        ...
        num_post = st.number_input("Number of items", min_value=0, value=0, key="num_post")
        items_post_data = []
        for i in range(num_post):
            st.markdown(f"**Item {i+1}**")
            conn_type_raw = st.selectbox("Type", ["Argument", "Periphery (NUC)", "Periphery (CORE)", "Periphery (CLAUSE)"], key=f"post_c_{i}")
            code, def_lbl = conn_map[conn_type_raw]
            c1, c2, c3 = st.columns([1, 2, 1])
...
with main_c2:
    ...
    if show_graph:
        ...
        try:
            ...
            # Buttons: "Download" on the left, "New" on the right
            ...
            with btn_col3:
                if st.button("New", use_container_width=True, help="Generate new diagram"):
